...
SCHEMA_FILE     := provider/cmd/pulumi-resource-netcup/schema.json
...
prepare:
	...
	@if test ! -d "provider/cmd/pulumi-resource-netcup"; then "Project already prepared"; exit 1; fi # SED_SKIP

	# SED needs to not fail when encountering unicode characters
	LC_CTYPE=C 
	LANG=C

	mv "provider/cmd/pulumi-resource-netcup" provider/cmd/pulumi-resource-${NAME} # SED_SKIP
...
sign-goreleaser-exe-%: bin/jsign-6.0.jar
	...
	if [[ "${SKIP_SIGNING}" != "true" ]]; then \
		if [[ "|${AZURE_SIGNING_CLIENT_ID}|${AZURE_SIGNING_CLIENT_SECRET}|${AZURE_SIGNING_TENANT_ID}|${AZURE_SIGNING_KEY_VAULT_URI}|" == *"||"* ]]; then \
			...
			file=dist/build-provider-sign-windows_windows_${GORELEASER_ARCH}/pulumi-resource-netcup.exe; \
